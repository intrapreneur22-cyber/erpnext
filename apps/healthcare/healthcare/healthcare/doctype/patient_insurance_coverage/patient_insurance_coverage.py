...
class PatientInsuranceCoverage(Document):
	def validate(self):
		...
		if self.mode_of_approval == "Manual" and self.status == "Draft" and not self.flags.silent:
			frappe.msgprint(
				_("Manual approval required for Insurance Coverage {}").format(self.name),
				alert=True,
				indicator="orange",
			)
	...
	def validate_insurance_policy(self):
		if not self.insurance_policy:
			frappe.throw(
				_("Patient Insurance Policy is required to create Insurance Coverage"),
				title=_("Missing Insurance Policy"),
			)

		if not is_insurance_policy_valid(
			self.insurance_policy, self.posting_date, self.company
		):  # also checks for valid contract
			frappe.throw(
				_("Patient Insurance Policy {} is not valid as on {}").format(
					frappe.bold(self.insurance_policy), self.posting_date
				),
	...
	def before_submit(self):
		if self.status not in ["Approved", "Rejected"]:
			frappe.throw(
				_("Only Insurance Coverages in Status <b>Approved</b> or <b>Rejected</b> can be submitted"),
				title=_("Not Allowed"),
			)

		if self.status == "Approved" and self.coverage <= 0:
			frappe.throw(
				_("Invalid Coverage Percent {}, cannot submit Insurance Coverage as Approved").format(
					frappe.bold(self.coverage)
				),
				title=_("Not Allowed"),
			)
	...
	def on_submit(self):
		if not self.flags.silent:
			frappe.msgprint(
				_("Insurance Coverage {} - {}<br>Discount: {}%, Coverage: {}%").format(
					self.name, frappe.bold(self.status), self.discount, self.coverage
				),
				alert=True,
				indicator="green" if self.status == "Approved" else "orange",
			)
	...
	def set_and_validate_template_details(self):
		...
		if self.template_dt and self.template_dn and self.template_dt != "Appointment Type":
			...
			if not self.item_code:
				frappe.throw(
					_("Invalid Service Template, Item is required to create Insurance Coverage"),
	...
	def set_insurance_coverage(self):
		...
		if not eligibility:
			frappe.msgprint(
				_("Insurance Eligibility not found for Item {}.").format(self.item_code),
				alert=True,
				indicator="error",
			)
...
def get_item_price_list_rate(item_code, price_list, qty, company):
	# v15-compatible approach - pass args dict instead of ItemDetailsCtx
	args = frappe._dict({
